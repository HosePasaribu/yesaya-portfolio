        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {writeups.map((w, i) => (
            <Reveal key={w.title} delay={i * 0.08}>
              <article className="card-glow group h-full rounded-xl border border-border bg-card/60 p-6 cursor-pointer">
                ...
                <h3 className="mt-4 font-semibold text-white leading-snug group-hover:text-neon transition-colors">
                  {w.title}
                </h3>
                ...
                <div className="mt-3 inline-flex items-center gap-1 font-mono text-xs text-muted group-hover:text-neon transition-colors">
                  read write-up <ArrowUpRight className="h-3.5 w-3.5" />
                </div>
              </article>
            </Reveal>
          ))}
